			 	 ...
			 	 __write_only image3d_t cache)
{
	...
	VECTORTYPE result = (uint4){val,0,0,0};
			 	 ...
			 	 __write_only image3d_t cache)
{
	...
	for (int d3=0; d3<3; d3++)
	{
		for (int d2=0; d2<3; d2++)
		{
			...
			{
				int index = d1+d2*3+d3*9;
				VECTORTYPE val = READ_IMAGE(image1, sampler, (pos-(int4){1,1,1,0}+(int4){d1,d2,d3,0}));
				res = res+val.x;
			}
		}
	}
	...
	VECTORTYPE result = (uint4){res,0,0,0};
